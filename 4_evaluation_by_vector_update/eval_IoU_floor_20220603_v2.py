...
class Eval_byIoU():
    ...
    def create_rect(self, lines): # 基于构件轴线坐标创建矩形，用于后续交并比计算
        contours,contour_areas,line_lens = [],[],0
        for i,line in enumerate(lines):
            ...
            line_lens = line_lens + line_len

        return contours,contour_areas,line_lens
    ...
    def iou_eval(self):
        ...
        GAN_contours,GAN_contour_areas,GAN_line_lens = self.create_rect(self.GAN_structs)
        # 获取工程师设计的剪力墙构件轴线对应的矩形
        Eng_contours,Eng_contour_areas,Eng_line_lens = self.create_rect(self.Eng_structs)
        ...
        # 剪力墙数量差异计算
        ita_lens = 1-abs(GAN_line_lens-Eng_line_lens)/Eng_line_lens
        ...
        if len(inter_areas) == 0:
            ...
            Shearwall_IoU_M = Shearwall_IoU*ita_lens
        ...
        with open(txt_dir,"w") as iou_txt:
            iou_txt.write("%.8f\n"%Shearwall_IoU_M)
            iou_txt.write("Shearwall_IoU: %.8f\n"%Shearwall_IoU)
